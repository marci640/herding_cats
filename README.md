...
- If assumptions require TPM review, they are published to the Confluence `assumptions` page discovered from `ACTIVE_JIRA_EPIC` in the `SUDS` space. `TEAM INPUT` remains human-owned and `AI OUTPUT` is the agent-managed review surface.
...
- On resume, the agent fetches the latest assumptions from Confluence, treating the full current page state as truth when it is marked `ready`, and syncs locally before continuing.
...
| Action | Command |
|---|---|
| Draft requirements from Jira + Confluence | *"Read the ACTIVE_JIRA_EPIC from .env, fetch the matching Jira epic, and read the Confluence `requirements` page discovered in `SUDS` via `title ~ \"requirements\" AND text ~ \"${ACTIVE_JIRA_EPIC}\"`. Draft `.ai/SPRINT_REQUIREMENTS.md` from both."* |
...
| Requirements revised on Confluence | *"Requirements updated. Re-run from Phase 1."* *(re-fetch the `requirements` page discovered from `ACTIVE_JIRA_EPIC`, update business-source sections, preserve compatible AI-generated requirements unless superseded)* |
